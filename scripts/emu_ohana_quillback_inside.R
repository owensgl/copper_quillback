...
###Emu plotting
missing_data <- read_table("/project/ctb-grego/ntbsykes/copper_quillback/vcf/inside_quillback/combined.quillback.inside.imiss") %>%
  select(INDV, F_MISS) %>%
  rename(sample = INDV)
emu_pca <- read_tsv("/project/ctb-grego/ntbsykes/copper_quillback/vcf/inside_quillback/combined.quillback.inside.emu.eigvecs") %>%
  select(-`#FID`) %>%
  rename(sample = IID) %>%
  inner_join(sample_info)

eigenvalues <- scan("/project/ctb-grego/ntbsykes/copper_quillback/vcf/inside_quillback/combined.quillback.inside.emu.eigvals")
...
samples <- read_tsv("/project/ctb-grego/ntbsykes/copper_quillback/structure/quillback_inside/combined.quillback.inside.nosex",
...
for (k in 2:4){
  data <- read.delim(paste0("/project/ctb-grego/ntbsykes/copper_quillback/vcf/inside_quillback/combined.quillback.inside.",k,".q.matrix"),
                     header = FALSE, skip=1,
...
make_phylo_plot <- function(k_val) {
  ...
  file_path <- paste0("/project/ctb-grego/ntbsykes/copper_quillback/vcf/inside_quillback/combined.quillback.inside.", k_val, ".tree.nwk")
...
pdf("/project/ctb-grego/ntbsykes/copper_quillback/plots/quillback_inside.emu.ohana.pdf",
    height=8,width=6)
print(final_plot)
dev.off()
