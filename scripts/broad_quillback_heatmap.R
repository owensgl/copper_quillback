...
get_cluster_order <- function(data) {
  ...
  # 'euclidean' is standard, but 'pearson' correlation distance often 
  # works better for D-stats to show similar patterns of relationship
  dist_matrix <- dist(matrix_data)
  
  # 3. Perform Hierarchical Clustering
  clustering <- hclust(dist_matrix, method = "average") # "average" often produces smoother transitions
  
  # 4. Optimal Leaf Ordering (Optional but highly recommended)
  # This flips branches to ensure adjacent items are as similar as possible
  # If you don't want to install 'vegan', you can skip this step, but it helps.
  dd <- as.dendrogram(clustering)
  # Reorder based on the first principal component of the distance matrix
  optimal_order <- reorder(dd, rowMeans(matrix_data))
  
  # 5. Extract the order
  ordered_samples <- labels(optimal_order)
  
  # Return the ranked dataframe
  tibble(
    P2 = ordered_samples,
    cluster_rank = 1:length(ordered_samples)
  )
}
...
outliers <- plot_data %>%
  ...
  mutate(`p-value` = case_when(is.na(`p-value`) ~ 0,
                               T ~ `p-value`)) %>%
  mutate(outlier = case_when(deviation >= threshold_D & Dstatistic > 0 & `p-value` < 0.0001 & `f4-ratio` > 0.01 ~ "outlier",
                             T ~ NA)) %>%
...
# Flip branches if necessary
#p_tree <- flip(p_tree, 56, 68)

# 4. Combine (Tree on Top, Heatmap with labels on Bottom)
...
pdf("/project/ctb-grego/ntbsykes/copper_quillback/plots/broad_quillback.heatmap.v2.pdf",
    height=12,width=5)
final_plot
dev.off()
...
bar_plot <- plot_data_heatmap %>%
  full_join(outliers) %>%
  mutate(chosen_outlier = case_when(deviation == max_dev  ~ "chosen",
                                    T ~ "not_chosen")) %>%
  ...
  scale_fill_manual(values = c("Aurora" =palette_1[1],
                               "Copper" = "#daa038",
                               "Greenstriped" = palette_2[6],
                               "Greenblotched" = palette_1[3],
                               "Canary" = palette_2[1],
                               "Widow" = palette_2[4],
                               "Redstripe" = palette_2[3],
                               "Light_dusky" = palette_1[5],
                               "Northern" = palette_2[7],
                               "Blue" = palette_1[6],
                               "Rougheye" = palette_1[7],
                               "Yelloweye" = palette_2[9]
  ),
    name="Donor Species",
    breaks = c("Aurora","Blue", "Canary", "Copper","Greenblotched","Greenstriped",
               "Light_dusky","Northern","Redstripe","Rougheye","Yelloweye","Widow"),
    na.translate = F) +
  theme_cowplot() +
...
pdf("/project/ctb-grego/ntbsykes/copper_quillback/plots/broad_quillback.proportions.v1.pdf",
    height=12,width=3)
bar_plot
dev.off()
