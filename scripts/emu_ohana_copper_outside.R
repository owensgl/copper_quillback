...
###Emu plotting
missing_data <- read_table("/project/ctb-grego/ntbsykes/copper_quillback/vcf/outside_copper/combined.copper.outside.imiss") %>%
  select(INDV, F_MISS) %>%
  rename(sample = INDV)
emu_pca <- read_tsv("/project/ctb-grego/ntbsykes/copper_quillback/vcf/outside_copper/combined.copper.outside.emu.eigvecs") %>%
  select(-`#FID`) %>%
  rename(sample = IID) %>%
  inner_join(sample_info)

eigenvalues <- scan("/project/ctb-grego/ntbsykes/copper_quillback/vcf/outside_copper/combined.copper.outside.emu.eigvals")
...
pdf("/project/ctb-grego/ntbsykes/copper_quillback/plots/copper_outside_geography.pdf",
    height = 5,width=5)
emu_pca %>%
  filter(pop_short %in% c("WA/OR","CA")) %>%
  ggplot(.,aes(y=PC2, x=latitude)) +
  geom_point() +
  geom_vline(xintercept = 34.5,linetype="dotted") +
  geom_smooth(se=F, method="lm") +
  theme_cowplot() +
  ylab("Coastal Copper PC2") +
  xlab("Latitude (N)")
dev.off()
pca_2 <- emu_pca %>%
  inner_join(missing_data) %>%
...
samples <- read_tsv("/project/ctb-grego/ntbsykes/copper_quillback/structure/copper_outside/combined.copper.outside.nosex",
...
for (k in 2:10){
  data <- read.delim(paste0("/project/ctb-grego/ntbsykes/copper_quillback/vcf/outside_copper/combined.copper.outside.",k,".q.matrix"),
                     header = FALSE, skip=1,
...
make_phylo_plot <- function(k_val) {
  ...
  file_path <- paste0("/project/ctb-grego/ntbsykes/copper_quillback/vcf/outside_copper/combined.copper.outside.", k_val, ".tree.nwk")
...
all_rows <- lapply(2:4, function(k) {
  p_admix <- make_admix_plot(copper_admix, k)
...
all_rows <- lapply(2:5, function(k) {
  p_admix <- make_admix_plot(copper_admix, k)
  p_phylo <- make_phylo_plot(k)
...
pdf("/project/ctb-grego/ntbsykes/copper_quillback/plots/copper_outside.emu.ohana.pdf",
    height=8,width=6)
print(final_plot)
dev.off()