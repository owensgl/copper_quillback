...
missing_data <- read_table("/project/ctb-grego/ntbsykes/copper_quillback/vcf/inside_copper/combined.copper.inside.imiss") %>%
  select(INDV, F_MISS) %>%
  rename(sample = INDV)
emu_pca <- read_tsv("/project/ctb-grego/ntbsykes/copper_quillback/vcf/inside_copper/combined.copper.inside.emu.eigvecs") %>%
  select(-`#FID`) %>%
  rename(sample = IID) %>%
  inner_join(sample_info)

eigenvalues <- scan("/project/ctb-grego/ntbsykes/copper_quillback/vcf/inside_copper/combined.copper.inside.emu.eigvals")
...
for (k in 2:10){
  data <- read.delim(paste0("/project/ctb-grego/ntbsykes/copper_quillback/vcf/inside_copper/combined.copper.inside.",k,".q.matrix"),
                     header = FALSE, skip=1,
...
make_phylo_plot <- function(k_val) {
  ...
  file_path <- paste0("/project/ctb-grego/ntbsykes/copper_quillback/vcf/inside_copper/combined.copper.inside.", k_val, ".tree.nwk")
...
pdf("/project/ctb-grego/ntbsykes/copper_quillback/plots/copper_inside.emu.ohana.pdf",
    height=8,width=6)
print(final_plot)
dev.off()
